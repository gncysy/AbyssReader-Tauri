            ...
            <div class="app-body">
              ...
              <main class="app-main">
                ...
                <BookDetail
                  ...
                  :book="bookshelfStore.detailBook ? JSON.parse(JSON.stringify(bookshelfStore.detailBook)) : null"
                  :source="bookshelfStore.detailSource ? JSON.parse(JSON.stringify(bookshelfStore.detailSource)) : null"
                  @close="bookshelfStore.closeDetail()"
                />
                <Reader
                  v-if="bookshelfStore.showReader"
                  :book="bookshelfStore.readerBook ? JSON.parse(JSON.stringify(bookshelfStore.readerBook)) : null"
                  :source="bookshelfStore.readerSource ? JSON.parse(JSON.stringify(bookshelfStore.readerSource)) : null"
...
import {
  BookOutline, SearchOutline, CompassOutline,
  CloudOutline, SettingsOutline, AppsOutline
...
function navigate(routeName: string) {
  if (routeName !== currentRoute.value) router.push({ name: routeName }).catch(() => {})
}
...
.titlebar {
  ...
  background: var(--bg-card); flex-shrink: 0; z-index: 20;
}
.titlebar-drag { flex: 1; height: 100%; user-select: none; -webkit-app-region: drag; }
.titlebar-controls {
  display: flex; gap: 0; flex-shrink: 0; -webkit-app-region: no-drag;
}
.titlebar-btn {
...
.nav-item {
  ...
  transition: background 0.2s, color 0.2s; color: var(--text-muted);
}
