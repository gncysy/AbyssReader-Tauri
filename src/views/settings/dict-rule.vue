...
async function loadRules(): Promise<void> { try { rules.value = (await store.get('dictRule')) || [] } catch { rules.value = [] } }
...
function editRule(rule: any): void { editingRule.value = rule; form.value = { name: rule.name, urlRule: rule.urlRule, showRule: rule.showRule || '', enabled: rule.enabled }; showDialog.value = true }
async function saveRule(): Promise<void> { if (!form.value.name.trim() || !form.value.urlRule.trim()) { msg.warning('名称和 URL 不能为空'); return }; const list = [...rules.value]; if (editingRule.value) { const idx = list.findIndex((r: any) => r.name === editingRule.value.name); if (idx !== -1) list[idx] = { ...editingRule.value, ...form.value } } else list.push({ ...form.value, sortNumber: list.length }); rules.value = list; await saveRules(); showDialog.value = false; msg.success('已保存') }
async function deleteRule(rule: any): Promise<void> { rules.value = rules.value.filter((r: any) => r.name !== rule.name); await saveRules(); msg.success('已删除') }
async function toggleRule(rule: any): Promise<void> { rule.enabled = !rule.enabled; await saveRules() }
</script>
