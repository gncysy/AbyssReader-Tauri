<template>
  <div class="settings-page">
    <header class="page-header"><h1 class="page-title">设置</h1><p class="page-subtitle">偏好与配置</p></header>
    <div style="display:flex;flex-direction:column;background:var(--bg-card);border:1px solid var(--border-color);border-radius:var(--radius-lg);overflow:hidden">
      <div v-for="item in menuItems" :key="item.path" class="settings-item" tabindex="0" @click="navigateTo(item.path)" @keydown.enter="navigateTo(item.path)">
        <div style="display:flex;flex-direction:column;flex:1"><span style="font-size:15px;font-weight:500;color:var(--text-primary)">{{ item.label }}</span><span style="font-size:13px;color:var(--text-muted);margin-top:2px">{{ item.desc }}</span></div>
...
const menuItems = [
  ...
  { path: '/settings/diagnostics', label: '诊断日志', desc: 'JS 执行诊断 / 书源兼容性分析' },
  { path: '/settings/about', label: '关于', desc: '版本信息' },
]
function navigateTo(path: string): void { router.push(path).catch(() => {}) }
</script>
