<template>
  <div class="rss-debug-page">
    <header class="subpage-header">
      <BackButton />
      <h2>订阅源调试</h2>
      <span v-if="source" class="source-name-badge">{{ source.sourceName }}</span>
      <div style="flex:1"></div>
      <button class="btn-secondary" style="padding:4px 12px;font-size:12px" @click="showHtmlModal = true">查看 HTML</button>
      <button class="btn-secondary" style="padding:4px 12px;font-size:12px" @click="clearLogs">清空日志</button>
    </header>
    ...
    <div class="debug-toolbar">
      <div class="toolbar-left">
        ...
        />
        <button class="btn-primary" :disabled="running" @click="runDebug">
          {{ running ? '执行中...' : '调试' }}
        </button>
      </div>
      <div class="toolbar-right">
        <label style="display:flex;align-items:center;gap:4px;font-size:12px;color:var(--text-muted)">
          <input type="checkbox" v-model="showDetailLog" /> 详细日志
        </label>
      </div>
    </div>
    ...
    <div class="log-container">
      ...
      <div class="log-list" ref="logListRef">
        ...
        >
          ...
          <span class="log-module">{{ log.module || 'debug' }}</span>
    ...
    <n-modal v-model:show="showHtmlModal" preset="card" title="HTML 源码" style="max-width:90vw;max-height:85vh" :bordered="false">
      <div class="html-viewer">
        <div class="html-toolbar">
          <span style="font-size:12px;color:var(--text-muted)">{{ htmlContent.length }} 字符</span>
          <button class="btn-secondary" style="padding:2px 10px;font-size:11px" @click="copyHtml">复制</button>
        </div>
        <textarea v-model="htmlContent" class="html-textarea" readonly spellcheck="false"></textarea>
      </div>
    </n-modal>
    ...
    <n-modal v-model:show="showResultModal" preset="card" title="解析结果" style="max-width:80vw;max-height:80vh" :bordered="false">
      <div class="result-viewer">
        <div class="result-toolbar">
          <span style="font-size:12px;color:var(--text-muted)">{{ resultItems.length }} 条</span>
          <button class="btn-secondary" style="padding:2px 10px;font-size:11px" @click="copyResult">复制</button>
        </div>
        <div class="result-list">
          <div v-for="(item, idx) in resultItems.slice(0, 100)" :key="idx" class="result-item">
            <span class="result-index">{{ idx + 1 }}</span>
            <span class="result-title">{{ item.title || '无标题' }}</span>
            <span v-if="item.link" class="result-link">{{ item.link }}</span>
          </div>
          <div v-if="resultItems.length > 100" style="padding:8px;text-align:center;color:var(--text-muted);font-size:12px">
            仅显示前 100 条，共 {{ resultItems.length }} 条
          </div>
        </div>
      </div>
    </n-modal>
...
const sortOptions = computed(() => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const name = lines[i].substring(0, idx)
    opts.push({ label: name, value: i })
  }
...
function addLog(level: string, message: string, module = 'rss-debug') {
  const now = new Date()
  const time = now.toTimeString().slice(0, 8)
  logs.value.push({ time, level, module, message, source: 'frontend' })
  nextTick(() => {
    if (logListRef.value) {
...
async function loadSource() {
  ...
  try {
    ...
    if (!source.value) {
      ...
      if (sortOptions.value.length > 0 && sortOptions.value[0].value !== -1) {
        selectedSortIndex.value = sortOptions.value[0].value
      }
    }
  } catch (err: any) {
...
async function runDebug() {
  if (!source.value) {
    message.warning('请先加载订阅源')
    return
  }

  const keyword = searchInput.value.trim()
  if (!keyword) {
    message.warning('请输入关键词或 URL')
    return
  }
    ...
    if (!targetUrl.startsWith('http://') && !targetUrl.startsWith('https://')) {
      if (selectedSortIndex.value >= 0 && source.value.sortUrl) {
        ...
        if (selectedSortIndex.value < lines.length) {
          ...
          const url = lines[selectedSortIndex.value].substring(idx + 2)
          targetUrl = url.replace(/\{\{key\}\}/g, encodeURIComponent(keyword))
        }
      }
    }
...
function copyResult() {
  navigator.clipboard.writeText(JSON.stringify(resultItems.value, null, 2))
  message.success('已复制')
}
