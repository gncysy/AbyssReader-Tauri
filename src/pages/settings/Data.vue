<template>
  <div class="settings-subpage">
    ...
    <div class="cache-section">
      <div class="setting-item">
        <div>
          <span class="label-text">本地缓存</span>
          <span class="label-desc">书籍封面、目录、正文及漫画图片缓存</span>
        </div>
        <button class="btn-secondary" :disabled="loadingCache" @click="loadCacheInfo">
          {{ loadingCache ? '加载中...' : '刷新' }}
        </button>
      </div>
      ...
      <div v-if="cacheInfo" class="cache-info">
        <div class="cache-summary">
          <div class="cache-stat">
            <span class="cache-stat-label">缓存位置</span>
            <span class="cache-stat-value cache-path">{{ cacheInfo.path }}</span>
          </div>
          <div class="cache-stat">
            <span class="cache-stat-label">总大小</span>
            <span class="cache-stat-value">{{ cacheInfo.totalSizeFormatted }} / {{ cacheInfo.maxTotalFormatted }}</span>
          </div>
          <div class="cache-stat">
            <span class="cache-stat-label">缓存上限</span>
            <div class="cache-limit-row">
              ...
              />
              <span class="cache-stat-value">MB</span>
              <button class="btn-secondary" style="padding:2px 10px;font-size:11px" :disabled="savingLimit" @click="saveCacheLimit">
                {{ savingLimit ? '保存中' : '保存' }}
              </button>
            </div>
          </div>
          <div class="cache-stat">
            <span class="cache-stat-label">迁移目录</span>
            <button class="btn-secondary" style="padding:4px 12px;font-size:12px" :disabled="migrating" @click="migrateCache">
              {{ migrating ? '迁移中...' : '选择新目录' }}
            </button>
          </div>
        </div>

        <div class="cache-categories" v-if="cacheInfo.categories && cacheInfo.categories.length > 0">
          <div v-for="cat in cacheInfo.categories" :key="cat.name" class="cache-category-row">
            <div class="cache-cat-info">
              <span class="cache-cat-name">{{ cat.name }}</span>
              <span class="cache-cat-detail">{{ cat.count }} 个文件 · {{ cat.sizeFormatted }}</span>
            </div>
            <button
              class="btn-secondary"
              style="padding:4px 12px;font-size:12px"
              :disabled="clearingCategory === cat.name"
              @click="clearCategory(cat.name)"
            >
              {{ clearingCategory === cat.name ? '清理中...' : '清理' }}
            </button>
          </div>
        </div>

        <div class="cache-actions">
          <button class="btn-danger" :disabled="clearingAll" @click="clearAllCache">
            {{ clearingAll ? '清空中...' : '清空所有缓存' }}
          </button>
        </div>
      </div>
    </div>
...
async function loadCacheInfo() {
  ...
  try {
    cacheInfo.value = await invoke('cache_get_info')
    cacheLimitMB.value = Math.round((cacheInfo.value.maxTotalBytes || 200 * 1024 * 1024) / (1024 * 1024))
  } catch (err: any) {
    message.error('加载缓存信息失败: ' + (err?.message || String(err)))
  } finally { loadingCache.value = false }
}

async function saveCacheLimit() {
  savingLimit.value = true
  try {
    await invoke('cache_set_max_size', { maxMb: cacheLimitMB.value })
    await loadCacheInfo()
...
async function migrateCache() {
  ...
  try {
    // 通过打开目录选择（简化：弹出输入框）
    const newPath = prompt('请输入新的缓存目录路径（如 D:\\Cache\\moyue）:')
    if (!newPath) return
    await invoke('cache_migrate', { newPath })
    await loadCacheInfo()
    message.success('缓存目录已迁移')
  } catch (err: any) {
    message.error('迁移失败: ' + (err?.message || String(err)))
  } finally { migrating.value = false }
}

async function clearCategory(name: string) {
  const catMap: Record<string, string> = { '封面': 'covers', '目录': 'toc', '正文': 'content', '漫画': 'comic' }
  clearingCategory.value = name
  try {
    const result: any = await invoke('cache_clear_category', { category: catMap[name] || name })
    message.success(`已清理 ${result.removed} 个文件`)
    await loadCacheInfo()
  } catch (err: any) { message.error('清理失败: ' + (err?.message || String(err))) }
  finally { clearingCategory.value = null }
}

async function clearAllCache() {
  clearingAll.value = true
  try {
    const result: any = await invoke('cache_clear')
    message.success(`已清空 ${result.removed} 个缓存文件`)
    await loadCacheInfo()
  } catch (err: any) { message.error('清空失败: ' + (err?.message || String(err))) }
