<template>
  <div class="explore-page">
    ...
    <div v-if="!loading && books.length > 0 && !hasMore" style="text-align:center;padding:16px 0;color:var(--text-muted);font-size:13px">
      — 已加载全部 —
    </div>

    <div v-else-if="!loading && !loadingCategories && books.length === 0 && categories.length > 0 && currentCategory" class="empty-state" style="padding:40px 0">
      <h3>暂无书籍</h3>
      <p style="color:var(--text-muted);font-size:13px">该分类下没有找到内容</p>
    </div>

    <div v-else-if="!loading && !loadingCategories && categories.length > 0 && !currentCategory" class="empty-state" style="padding:40px 0">
      <h3>选择分类</h3>
...
import { onLog, logHistory, logInfo, logError, type LogEntry } from '../../engine/event/index.js'
...
function getInfoMapValue(cat: ExploreKind): string {
  return infoMapStore.get(currentSourceUrl.value, cat.title)
}

function setInfoMapValue(cat: ExploreKind, value: string): void {
  infoMapStore.set(currentSourceUrl.value, cat.title, value)
}
...
async function executeAction(action: string) {
  try {
    ...
    const sandbox = buildSandbox('', '', currentSourceUrl.value)
    sandbox.sourceUrl = currentSourceUrl.value
    sandbox.infoMap = infoMapStore
    sandbox.setInfoMap = (key: string, val: string) => infoMapStore.set(currentSourceUrl.value, key, val)
    sandbox.getInfoMap = (key: string) => infoMapStore.get(currentSourceUrl.value, key)
    sandbox.refreshExplore = () => loadCategories()
    executeJs(processed, sandbox)
  } catch (e) {
...
async function loadBooks() {
  ...
  try {
    ...
    url = url.replace(/\{\{if\(page==1\)\{([^}]*)\}\}\}/g, (_, expr) => {
      if (currentPage.value === 1) return expr || ''
      return ''
    })
...
function setupLoadMoreObserver() {
  ...
  if (currentSentinel && currentSentinel.parentNode) {
