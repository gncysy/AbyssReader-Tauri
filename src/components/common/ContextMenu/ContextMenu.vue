...
function open(_items: any[], x: number, y: number): void {
  visible.value = true
  menuStyle.left = x + 'px'
  menuStyle.top = y + 'px'
  if (menuRef.value) {
...
function onEnter(el: Element): void {
  ...
  if (rect.right > vw - padding) {
    menuStyle.left = (parseFloat(menuStyle.left) - rect.width) + 'px'
    if (parseFloat(menuStyle.left) < padding) menuStyle.left = padding + 'px'
  }
  if (parseFloat(menuStyle.left) < padding) menuStyle.left = padding + 'px'

  if (rect.bottom > vh - padding) {
    menuStyle.top = (parseFloat(menuStyle.top) - rect.height) + 'px'
    if (parseFloat(menuStyle.top) < padding) menuStyle.top = padding + 'px'
  }
